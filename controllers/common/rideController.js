...
const getDropPoints = async (req, res) => {
  ...
  try {
    const airports = await db.airport_fare.findAll({
      ...
      where: { pickup_location: pickupPoint },
      raw: true,
    });
    const railways = await db.railway_fare.findAll({
      attributes: ["drop_location", "fare"],
    ...
    const dropPoints = [
      ...airports.map((airport) => ({ drop_point: airport.drop_location, fare: airport.fare })),
      ...railways.map((railway) => ({ drop_point: railway.drop_location, fare: railway.fare })),
    ];
